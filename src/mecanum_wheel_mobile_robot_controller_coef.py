...
with open(filename, 'rb') as csvfile:
    ...
    a5x_list = []
    for row in reader:
        a0, a1, a2, a3, a4, a5 = float(row[0]), float(row[1]), float(row[2]), float(row[3]), float(row[4]), float(
            row[5])
        a0x_list.append(a0)
        a1x_list.append(a1)
        a2x_list.append(a2)
...
def x_trajectory(time):
    ...
    mapper = interp1d([interval_index * time_interval, (interval_index + 1) * time_interval], [0, time_interval])
    time = mapper(time)

    # Returning the value of the polynomial evaluated at that time
    return a0 + a1 * time + a2 * math.pow(time, 2) + a3 * math.pow(time, 3) + a4 * math.pow(time, 4) + a5 * math.pow(
        time, 5)
